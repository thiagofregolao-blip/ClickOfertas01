// CÓDIGO DA RASPADINHA - ANÁLISE TÉCNICA
// Gerado em: 27/08/2025
...
export default function ScratchCard({ product, currency, themeColor, onRevealed, onClick }: ScratchCardProps) {
  ...
  const handleScratch = (clientX: number, clientY: number) => {
    ...
    // CONFIGURAÇÃO FINAL: Raio 25px para raspagem natural
    ...
    try {
      ...
      // Som não disponível (modo silencioso)
    }

    // Registrar área raspada
    scratchedAreas.current.push({ x, y, radius: scratchRadius });

    // ALGORITMO DE SCRATCH - Natural e sólido
    ctx.globalCompositeOperation = 'destination-out';
    ctx.fillStyle = 'rgba(0,0,0,1)'; // Totalmente opaco
    ctx.beginPath();
    ...
    // THRESHOLD FINAL: 70% para revelação natural
  ...
  return (
    <Card className="relative border-2 border-yellow-400 bg-gradient-to-br from-yellow-100 to-orange-100 shadow-lg cursor-pointer select-none">
      <CardContent className="p-0 relative h-48">
        ...
        {/* Canvas de scratch - ELEMENTO PRINCIPAL */}
        <canvas
          ref={canvasRef}
        ...
        {/* EFEITO GRADUAL DO DESCONTO - Z-INDEX BAIXO PARA NÃO INTERFERIR */}
...
- Canvas HTML5 para desenho e scratch
- Touch events para mobile
- Sistema de throttle (60fps)
- Web Audio API para efeitos sonoros
- Cálculo de progresso de raspagem
- Timer countdown após revelação
- Integração com backend (API mutations)
- Responsivo e acessível

🎯 CONFIGURAÇÕES FINAIS:
- Raio de scratch: 25px (natural)
- Threshold: 70% (experiência equilibrada)
- Throttle: 16ms (~60fps)
- Som: Frequência 150-200Hz, tipo sawtooth
- Z-index: Elementos informativos em z-0 (não interferem)

🔧 CORREÇÕES APLICADAS:
- Removida barra de progresso que interferia
- Som sintético gerado via Web Audio
- Scratch sólido sem gradientes
- Threshold balanceado (não muito fácil/difícil)
- Canvas com eventos touch otimizados

📱 COMPATIBILIDADE:
- Desktop: Mouse events
- Mobile: Touch events com preventDefault
- PWA: Funciona offline
- iOS/Android: Testado e funcional

🎨 UX/UI:
- Visual dourado atrativo
- Animações suaves
- Feedback visual gradual (?? → ???? → desconto)
- Timer com urgência
