      ...
      <div className="space-y-6">
        ...
        <Tabs defaultValue="manage" className="w-full">
          <TabsList className="grid w-full grid-cols-2">
            ...
            <TabsTrigger value="product-bank" className="flex items-center gap-2">
        ...
        <Card>
          ...
          {/* Tab Content - Product Bank Selection */}
          <TabsContent value="product-bank" className="space-y-6">
            <ProductBankSelector storeId={store.id} />
          </TabsContent>
        </Tabs>
      </div>
...
// Component for selecting and adding products from product bank
function ProductBankSelector({ storeId }: { storeId: string }) {
  const { toast } = useToast();
  const queryClient = useQueryClient();
  ...
  // Query for active product banks
  const { data: productBanks, isLoading: banksLoading } = useQuery({
    queryKey: ['/api/product-banks/active'],
    queryFn: async () => {
      const response = await fetch('/api/product-banks/active', {
        credentials: 'include',
      });
      if (!response.ok) {
        throw new Error('Erro ao carregar bancos de produtos');
      }
      return response.json();
    }
  });

  // Query for bank items
  const { data: bankItems, isLoading: itemsLoading, refetch: refetchItems } = useQuery({
    queryKey: ['/api/product-banks', selectedBank, 'items'],
    queryFn: async () => {
      if (!selectedBank) return { items: [] };
      
      const params = new URLSearchParams();
      if (searchTerm) params.append('search', searchTerm);
      
      const response = await fetch(`/api/product-banks/${selectedBank}/items?${params}`, {
        credentials: 'include',
      });
      if (!response.ok) {
  ...
  // Mutation to create product from bank
  const createProductMutation = useMutation({
    mutationFn: async ({ bankItemId, price }: { bankItemId: string; price: string }) => {
      const response = await fetch('/api/products/from-bank', {
        method: 'POST',
    ...
    onSuccess: (data) => {
      toast({
        ...
        description: `${data.product.name} foi adicionado à sua loja.`,
      });
  ...
  const handleAddProduct = (bankItemId: string, price: string) => {
    ...
    createProductMutation.mutate({ bankItemId, price });
  };
  ...
  }, [searchTerm, refetchItems, selectedBank]);

  if (banksLoading) {
    return <div className="text-center py-8">Carregando bancos de produtos...</div>;
  }
    ...
    return (
      <div className="text-center py-12">
        ...
        <h3 className="text-lg font-medium text-gray-900 mb-2">
          Nenhum banco de produtos disponível
        </h3>
        <p className="text-gray-500">
          Entre em contato com o administrador para adicionar produtos ao banco.
        </p>
      </div>
    );
  }
          ...
          <div className="space-y-4">
            <Select value={selectedBank} onValueChange={setSelectedBank}>
              <SelectTrigger data-testid="select-product-bank">
                <SelectValue placeholder="Escolha um banco de produtos..." />
              </SelectTrigger>
              <SelectContent>
                {productBanks.map((bank: any) => (
                  <SelectItem key={bank.id} value={bank.id}>
                    {bank.name} ({bank.totalProducts} produtos)
                  </SelectItem>
                ))}
              </SelectContent>
            </Select>
      ...
      {selectedBank && (
        <Card>
          ...
          <CardContent>
            ...
            ) : !bankItems?.items || bankItems.items.length === 0 ? (
              <div className="text-center py-8 text-gray-500">
                Nenhum produto encontrado
              </div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {bankItems.items.map((item: any) => (
                  <ProductBankCard
                    key={item.id}
                    item={item}
...
// Individual product card from bank
function ProductBankCard({ 
  ...
  item: any; 
  onAddProduct: (bankItemId: string, price: string) => void;
  isLoading: boolean;
}) {
    ...
    <div className="border rounded-lg overflow-hidden bg-white shadow-sm">
      ...
      <div className="p-4">
        ...
        <div className="space-y-2 text-sm text-gray-600 mb-4">
          {item.brand && <div><strong>Marca:</strong> {item.brand}</div>}
          {item.category && <div><strong>Categoria:</strong> {item.category}</div>}
          {item.color && <div><strong>Cor:</strong> {item.color}</div>}
          {item.storage && <div><strong>Armazenamento:</strong> {item.storage}</div>}
          {item.ram && <div><strong>RAM:</strong> {item.ram}</div>}
          <div><strong>Usado:</strong> {item.usageCount} vezes</div>
        </div>
        ...
        <div className="space-y-3">
          ...
          <Button
            onClick={() => onAddProduct(item.id, price)}
            disabled={!price || isLoading}
            className="w-full"
            data-testid={`button-add-product-${item.id}`}
          >
            {isLoading ? 'Adicionando...' : 'Adicionar à Loja'}
          </Button>
        </div>
