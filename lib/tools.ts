...
import { ensureIndex, semanticProducts, loadIndex } from './vectorStore';
...
export async function listStores(): Promise<Store[]> { return await readJson(STORES); }
export async function listHotels(): Promise<Hotel[]> { return await readJson(HOTELS); }
export async function listRestaurants(): Promise<Restaurant[]> { return await readJson(RESTS); }

/** Top lojas por categoria: slots Patrocinados + orgânico com boost de plano */
export async function topStores(options: { category?: string; limit?: number }): Promise<Store[]> {
  ...
  function baseScore(s: Store): number { return (s.rating || 0) + Math.min((s.reviews||0)/1000, 1); }
  function withBoost(s: Store): number {
...
export async function trendingProducts(options: { category?: string; limit?: number }): Promise<any[]> {
  ...
  items.sort((a,b)=>{
    const sa = storeMap.get(a.storeId); 
    const sb = storeMap.get(b.storeId);
...
function guessCategory(q: string): string | undefined {
...
export async function searchSuggestions(q: string): Promise<{
  category?: string;
  topStores: Store[];
  products: any[];
}> {
...
export async function buildItinerary(options: { wishlist?: any[] }): Promise<any> {
  const { wishlist = [] } = options;
  const categories = Array.from(new Set(wishlist.map((w: any) => w.category).filter(Boolean)));
  const storeBlocks = [];
