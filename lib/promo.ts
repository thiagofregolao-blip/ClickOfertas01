...
const lastShowMap = new Map<string, number>(); // key=userId|ip -> timestamp

function eligibleByCooldown(key: string): boolean {
  if (!key) return true;
...
async function callPromoEndpoint(params: { userId: string; ip: string; context: any }): Promise<any> {
  const { userId, ip, context } = params;
...
export async function maybeAttachPromo(params: { 
  payload: any; 
  userId: string; 
  ip: string; 
  context: any; 
}): Promise<any> {
  const { payload, userId, ip, context } = params;
  try {
